...
int main(int argc, char *argv[]) {
	...
	Texture texArray[4];
	//background texture
	texArray[0].load("..//..//Assets//Textures//lava.jpg ");
	texArray[0].setBuffers();
	texArray[1].load("..//..//Assets//Textures//sun.jpg");
	texArray[1].setBuffers();
	texArray[2].load("..//..//Assets//Textures//moon.jpg");
...
void handleWindowInput()
{
	...
	if (SDL_PollEvent(&event))
	{
		...
		if (event.type == SDL_KEYDOWN)
		{
			...
			//Added to allow the camera to move up or down
			if (event.key.keysym.sym == SDLK_e)
			{
				camera.ProcessKeyboard(UP, elapsedTime);
			}

			if (event.key.keysym.sym == SDLK_q)
			{
				camera.ProcessKeyboard(DOWN, elapsedTime);
			}
