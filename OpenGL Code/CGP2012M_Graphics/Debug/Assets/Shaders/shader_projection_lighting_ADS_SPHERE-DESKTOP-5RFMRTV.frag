...
{	//LIGHTING
	...
	//Testing with mixing textures
	//vec4 twotext = mix(textureColour, textureColour2, abs(sin(ovn*abs(sin(time/10000.0)))));
	
	//vec4 temp = mix(vec4(0.0, 1.0, 0.0, 1.0), vec4(1.0,0.0, 0.0, 1.0), abs(sin(ovn*abs(sin(time/10000.0)))));
	//vertColour = vec4(temp);
	
	//vertColour = step((vec4((ambient+diffuse+specular),1.0) * textureColour) + (vec4((ambient+diffuse+specular),1.0) * textureColour2));
	//vertColour = vec4((ambient+diffuse+specular),1.0) * textureColour2;
	//vertColour = vec4((ambient+diffuse+specular)*ovn,1.0);
	//vertColour = vec4(ovn*vec3(0.8,0.5,0.1) * (abs(sin(time/1000.0))+0.5), 1.0);
	//vertColour = (textureColour) + (textureColour2);
	//vertColour = mix(textureColour, textureColour2,0.5)

	//vertColour = step(textureColour, textureColour2) + abs(sin(time*2.5));
	//vertColour =  step(abs(sin(time*0.5)),textureColour);
	//vertColour = step(abs(sin(ovn*abs(sin(time/0.5)))), textureColour) + step(abs(sin(ovn*abs(sin(time/0.5)))),textureColour2);
	//vertColour =  smoothstep(0.0,1.0, textureColour);
	//float step = sin(time);
	//vertColour = textureColour * smoothstep(step, step, ovn) + textureColour2 * (1.0 - smoothstep(step, step, ovn));

	vertColour = mix(textureColour,textureColour2,smoothstep(ovn,0.0f,1.0f));
}