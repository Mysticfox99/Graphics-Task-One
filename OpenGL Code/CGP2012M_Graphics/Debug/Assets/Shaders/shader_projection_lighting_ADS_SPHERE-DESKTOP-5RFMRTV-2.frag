...
{	//LIGHTING
    ...
    vec3 erosion = smoothstep(t-0.2, t, ovn);
    
    vec3 border = smoothstep(0.0, 0.1, erosion) - smoothstep(0.1, 1.0, erosion);
    
    col = (1.0-erosion)*textureColour + erosion*textureColour2;
    
    vec3 leadcol = vec3(1.0, 0.5, 0.1);
    vec3 trailcol = vec3(0.2, 0.4, 1.0);
    vec3 fire = mix(leadcol, trailcol, smoothstep(0.8, 1.0, border))*2.;
	...
	//The closest I HAVE GOTTEN WITH THIS
	//vertColour = mix(textureColour,textureColour2,smoothstep(ovn,0.0f,1.0f));
	//vertColour = step(abs(sin(ovn)),textureColour) / step(abs(sin(ovn)),textureColour2);
