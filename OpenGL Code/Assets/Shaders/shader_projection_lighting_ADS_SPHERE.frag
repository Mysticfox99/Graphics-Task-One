...
{	//LIGHTING
	...
	//A time varaible that will allow chage between the two textures
	float threshold = sin(time/1000.0) *2; // Clip threshold
	
	//Comapre the threshold with the noise value
	float alpha = step(ovn, threshold) ; // alpha clip threshold
	
	vec4 borderColor = vec4(255.0, 255.0, 255.0, 0.0); //Assign a border color
	
	textureColour2 += borderColor * step(ovn - threshold, 0.25); // add the burn/border effect
	
	vertColour = (textureColour * alpha) + (textureColour2 * (1-alpha)); // blend the textures
